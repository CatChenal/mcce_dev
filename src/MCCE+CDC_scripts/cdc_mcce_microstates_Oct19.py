...
for filename in os.listdir(dir):
    ...
    if not os.path.isdir(os.path.join(dir, filename)) and filename != ".DS_Store":
        if filename.endswith(".pdb"):
            ...
            (
                dict_site_energy_shift_mcce,
                dict_total_contribution_mcce,
            ) = calculate_cdc_site_shift(
                protein_atomic, dielectric_eff=2.5, protein_contribution=True
            )
            dict_site_energy_mcc = calculate_total_site_energy(
                dict_site_energy_shift_mcce, 14950, 15674
            )

            md_object.create_dir("cdc_results")
            np.save(
                f"{md_scratch_dir}cdc_results/dict_total_contribution_mcce.npy",
                dict_total_contribution_mcce,
            )
