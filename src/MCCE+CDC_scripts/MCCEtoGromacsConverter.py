...
dict_resname_mapping = {
    "HIS": {"02": "HID", "01": "HIE", "+1": "HIP"},
    "HIL": {"01": "HID"},
...
class MCCEtoGromacsConverter:
    def __init__(
        self, mcce_pdb_path, list_all_ligands, dict_mapping_resname, saving_dir=None
    ):
        self.mcce_pdb_path = mcce_pdb_path
        self.all_ligands = (
            list_all_ligands  # list defining all the ligands in the mcce input file
        )
    ...
    def convert_mcce_to_gromacs(
        self,
        dict_update_protonation_residue=None,
        pdb_output_name="gromacs_most_occ_Mo.pdb",
    ):
        # 1. Convert mcce PDB format to extended PDB format:
        extended_pdb_1 = os.path.join(self.saving_dir, "extended_pdb1.pdb")
        self.convert_to_extended_pdb(self.mcce_pdb_path, extended_pdb_1)
    ...
    def convert_to_extended_pdb(self, mcce_pdb_path, extended_pdb_path):
        with open(mcce_pdb_path, "r") as mcce_pdb, open(
            extended_pdb_path, "w"
        ) as extended_pdb:
    ...
    def __construct_extended_pdb_line(self, mcce_line):
        (
            ...
            z_coord,
            element_symbol,
        ) = self.parse_pdb_line_string(mcce_line)
        if residue_name not in ["CLA", "CLB", "CHL", "CT1", "CT2", "CT3", "CT4"]:
            extended_line = (
                self.__construct_record_name_string(record_atom)
    ...
    def __construct_ligand_extended(
        self,
        record_atom,
        atom_index,
        atom_type,
        ...
        extended_line = (
            f"{self.__construct_record_name_string(record_atom)}"
    ...
    def rename_capping_residues(self, extended_pdb_path, complex_pdb_path):
        ...
        capping_residues = [
            ...
            "CT3",
            "CT4",
        ]  # List of capping residue names

        with open(extended_pdb_path, "r") as extended_pdb, open(
            complex_pdb_path, "w"
        ) as complex_pdb:
                ...
                if residue_name in capping_residues:
                    ...
                    with open(extended_pdb_path, "r") as pdb_file:
                        for pdb_line in pdb_file:
                            if (
                                pdb_line[21:26].strip() == residue_index
                                and pdb_line[17:20].strip() != residue_name
                            ):
        ...
        with open(pdb_file_path, "r") as pdb_file, open(
            ...
            for line in pdb_file:
                if line.startswith("ATOM") or line.startswith("HETATM"):
                    ...
                    if (
                        residue_name in mapping_dict
                        and key in mapping_dict[residue_name]
                    ):
    ...
    def rename_bk_atoms(self, pdb_file_path, output_file_path):
        dict_residue_info_mapping = self.__get_residue_info(
            input_pdb=pdb_file_path, output_pdb=output_file_path
        )
                        ...
                        if residue_id in dict_residue_info_mapping:
                            ...
                            for code in dict_residue_info_mapping[residue_id]:
                                if code != residue_code:
                                    new_residue_name = list(
                                        dict_residue_info_mapping[residue_id]
                                    )[0].split("_")[1]
                                    break
    ...
    def keep_protein_and_selected_ligands(
        self, pdb_file_path, output_file_path, ligands_to_keep
    ):
