...
list_all_ligands = [
    ...
    "HOH",
    "MEM",
]  # Should be updated based on the cofactors in the pdb
...
for filename in os.listdir(ms_mcce_pdb_dir):
    if filename != ".DS_Store":
        print(filename)
        converter = MCCEtoGromacsConverter(
            mcce_pdb_path=f"{ms_mcce_pdb_dir}{filename}",
        ...
        converter.convert_mcce_to_gromacs(
            pdb_output_name=f'{filename.split(".")[0]}_gromacs.pdb'
        )
