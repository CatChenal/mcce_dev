...
class MSout:
    ...
    def sort_microstates(
        self, sort_by: str = "E", sort_reverse: bool = False
    ) -> Union[list, None]:
...
def sort_charge_microstates(
    ...
    if sort_by not in ["count", "E", "total_E"]:
        print(
            "'sort_by' must be a valid charge_microstate attribute; choices: ['count', 'E', 'total_E']"
        )
        return None

    return sorted(
        charge_microstates, key=operator.attrgetter(sort_by), reverse=sort_reverse
    )


def topN_charge_microstates(
    charge_microstates: list,
    N: int = 1,
    sort_by: str = "count",
    ...
    sorted_charge_ms = sort_charge_microstates(
        charge_microstates, sort_by, sort_reverse
    )
